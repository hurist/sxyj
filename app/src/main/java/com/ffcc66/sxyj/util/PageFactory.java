...
public class PageFactory {
    ...
    private static Status mStatus = Status.OPENING;     //图书打开状态，默认为正在打开
...
//                    mContext.getResources(), R.drawable.main_bg, mWidth, mHeight));
            Bitmap bitmap = Bitmap.createBitmap(mWidth, mHeight, Bitmap.Config.RGB_565);    //创建指定格式、大小的位图作为背景图
            Canvas canvas = new Canvas(bitmap); //将背景位图传给Canvas（画布）
            canvas.drawColor(Color.BLACK);  //将背景画成黑色
            setBgBitmap(bitmap);        //将背景图传给类
            //设置字体颜色
            setM_textColor(Color.rgb(128, 128, 128));   //设置字体颜色
    ...
    private void drawStatus(Bitmap bitmap) {
        ...
        Canvas c = new Canvas(bitmap);  //将页面传给画布
        c.drawBitmap(getBgBitmap(), 0, 0, null);    //将背景图与图书页面图结合
        waitPaint.setColor(getTextColor());         //waitPaint加载界面的文字画笔
        waitPaint.setTextAlign(Paint.Align.CENTER);     //设置文字画的位置为居中

        Rect targetRect = new Rect(0, 0, mWidth, mHeight);  //Rect对象持有一个矩形的四个integer坐标值，这里创建了一个阅读页面大小的rect对象
        Paint.FontMetricsInt fontMetrics = waitPaint.getFontMetricsInt();       //获取字体属性对象
...
//        m_mbBufLen = 0;
        ...
        mStatus = Status.OPENING;       //图书打开状态设置为正在打开
        drawStatus(mBookPageWidget.getCurPage());       //获取当前页和下一页的bitmap
